...
public class Style
{
    private readonly GUIStyle _button = new GUIStyle()
    {
        fontSize = 12,
    ...
    {
        if (_button.normal != null && _button.normal.background != null) return;

        _button.normal = new GUIStyleState()
        {
            textColor = Color.white,
            background = CreateBorderTexture(new Color(0.2f, 0.2f, 0.2f, 0.5f))
        };
        _button.hover = new GUIStyleState()
        {
            textColor = Color.white,
            background = CreateBorderTexture(new Color(0.4f, 0.4f, 0.4f, 0.5f))
        };
        _button.active = new GUIStyleState()
        {
            textColor = Color.white,
            background = CreateBorderTexture(new Color(0.25f, 0.25f, 0.25f, 0.5f))
        };
        _button.onNormal = new GUIStyleState()
        {
            textColor = Color.yellow,
            background = CreateBorderTexture(new Color(0.25f, 0.25f, 0.25f, 0.5f))
        };
        _button.onHover = new GUIStyleState()
        {
            textColor = Color.yellow,
            background = CreateBorderTexture(new Color(0.4f, 0.4f, 0.4f, 0.5f))
        };
